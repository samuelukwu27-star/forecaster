...
# ============================================================
# EXTREMIZATION ENGINE (Includes Minibench 5-Trigger)
# ============================================================
...
class samcodes(ForecastBot):
    ...
    # -----------------------------
    # Custom aggregation + Minibench Extremization Engine
    # -----------------------------
        ...
        async with self._concurrency_limiter:
            ...
            if isinstance(question, BinaryQuestion):
                ...
                if g is not None and c is not None:
                    blend = clamp01(w_gpt * g + w_claude * c)
                    numeric_preds = [g, c]
                elif g is not None:
                    blend = clamp01(g)
                    numeric_preds = [g]
                ...
                stats_line = (
                    ...
                    f"sd={s:.3f} ci90=({lo:.3f},{hi:.3f}) agg=weighted(0.7/0.3) {ext_log}"
                )
            ...
            if isinstance(question, MultipleChoiceQuestion):
                ...
                ent = entropy(blended)
                stats_line = f"[stats] n_models={len(preds)} entropy={ent:.3f} agg=weighted(0.7/0.3) {ext_log}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default=["minibench", "32916", "market-pulse-26q2"],
    )
    ...
    async def run_all():
        ...
        for tid in args.tournament_ids:
            # Set the active tournament BEFORE firing the predictions
